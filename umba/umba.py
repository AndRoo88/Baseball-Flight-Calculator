...
can be changed for comparison. To retain the current value, just press return")
    ...
    while repeat == True:
        ...
        if QYangle == "":
            Yangle = Yangle #Yange is still in radians
        else:
            Yangle = float(QYangle)
            Yangle = (np.pi/180)*Yangle #converts to radians
        ...
        positions = (processing.PitchedBallTraj(x, y, z, Vtot, Theta, Psi, SpinRate, Tiltr, Gyro, Yangle, Zangle, i))
